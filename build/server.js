...
import { ANIMAL_INDEX, GRASS_ID, GROUND_INDEX, HUMAN_ID, WOLF_ID, RABBIT_ID, SHEEP_ID, THCIKGRASS_ID, E_LIGHTNING_ID, E_TSUNAMI_ID, E_GAMEOVER_1, E_GAMEOVER_2, E_GAMEOVER_3, E_METEORITE_FALL_HOT_ID, E_METEORITE_FALL_COOLED_ID, } from "./Constants/entities.js";
...
io.on("connection", (socket) => {
    ...
    socket.on("program-status", function (status) {
        switch (status) {
            case PROGRAM_RUN:
                Program.run();
                break;
    ...
    socket.on("debug-mode", (value) => {
        socket.data.debugMode = value;
    });
    socket.on("season", function (value) {
        Season.set(value);
    });
    socket.on("game-event", function ({ action, args }) {
        switch (action) {
            case "lightning":
    ...
    socket.on("chat", (username, text) => Chat.add(username, text));
});
